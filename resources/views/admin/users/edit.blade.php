...
@section('title', 'Edit Member | Sanctuary Admin')
@section('header', 'Update Member: ' . $user->name)
...
<div class="max-w-2xl">
    <div class="bg-surface-container-lowest p-8 rounded-2xl border border-outline-variant/10 shadow-sm">
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                <label for="password" class="text-xs font-bold uppercase tracking-widest text-on-surface-variant">New Password (Leave blank to keep current)</label>
            ...
            <div class="space-y-2">
                ...
                <select name="role" id="role" required
                    ...
                    <option value="instructor" {{ old('role', $user->role) === 'instructor' ? 'selected' : '' }}>Instructor (Owner)</option>
