...
@section('title', 'Buat Kelas Baru | Sanctuary Learning')
@section('header', 'Buat Kelas Baru')
    ...
    <div class="bg-surface-container-lowest p-8 rounded-2xl border border-outline-variant/15 shadow-sm">
        <form method="POST" action="{{ route('instructor.courses.store') }}" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="space-y-6">
                <!-- Title -->
                <div>
                    <label for="title" class="block text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-2">Judul Kelas</label>
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required autofocus
                        ...
                        placeholder="mis. Arsitektur Modern dan Etika">
                    @error('title')
                ...
                <!-- Description -->
                <div>
                    <label for="description" class="block text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-2">Deskripsi</label>
                    <textarea name="description" id="description" rows="5"
                        class="w-full px-4 py-3 bg-surface-container-low border-none rounded-xl focus:ring-2 focus:ring-primary/20 text-on-surface placeholder:text-on-surface-variant/40"
                        placeholder="Jelaskan apa yang akan dipelajari siswa di kelas ini...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-error font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <!-- Price -->
                    <div>
                        <label for="price" class="block text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-2">Harga (USD)</label>
                        <div class="relative">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-[10px] text-on-surface-variant/60">Ukuran maks: 2MB. Rasio yang disarankan: 16:9.</p>
                        @error('thumbnail')
                            <p class="mt-2 text-sm text-error font-medium">{{ $message }}</p>
                        @enderror
                    </div>
