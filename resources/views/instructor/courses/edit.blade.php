...
@section('title', 'Kelola Kurikulum | ' . $course->title)
@section('header', 'Kelola: ' . $course->title)
...
<div class="grid lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="flex justify-between items-center">
            <h3 class="text-xl font-bold tracking-tight">Kurikulum Kelas</h3>
        ...
        @if($course->chapters->isEmpty())
            <div class="bg-surface-container-low p-12 rounded-2xl border-2 border-dashed border-outline-variant/20 text-center">
                <p class="text-on-surface-variant font-medium">Kurikulum Anda kosong. Mulailah dengan menambahkan bab pertama Anda.</p>
            </div>
        @else
            <div class="space-y-4">
                @foreach($course->chapters as $chapter)
                    <div class="bg-surface-container-lowest border border-outline-variant/10 rounded-2xl overflow-hidden shadow-sm">
                        ...
                        <div class="bg-surface-container-low p-4 px-6 flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                <span class="text-xs font-bold text-on-surface-variant/50 uppercase tracking-widest">BAB {{ $loop->iteration }}</span>
                        ...
                        <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[10px] text-on-surface-variant uppercase tracking-widest font-bold mt-0.5">
                                                @if($sub->material || $sub->quiz)
                                                     <span class="text-primary">Konten Siap</span>
                                                @else
                                                     <span class="text-tertiary">Kosong</span>
                                                @endif
                                            </p>
                                        </div>
                                    </div>
                            ...
                            @empty
                                <p class="text-xs text-on-surface-variant italic p-4 text-center">Belum ada sub-bab.</p>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div id="add-sub-chapter-modal-{{ $chapter->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-on-surface/40 backdrop-blur-sm">
                        <div class="bg-surface-container-lowest p-8 rounded-2xl w-full max-w-md shadow-2xl border border-outline-variant/15">
                            <h3 class="text-xl font-bold mb-6 text-on-surface">Tambah Sub-bab ke {{ $chapter->title }}</h3>
                            <form action="{{ route('instructor.chapters.sub-chapters.store', $chapter->id) }}" method="POST" class="space-y-4">
                                @csrf
                                <div>
                                    <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2">Judul Sub-bab</label>
                                    <input type="text" name="title" required class="w-full px-4 py-3 bg-surface-container-low border-none rounded-xl focus:ring-2 focus:ring-primary/20" placeholder="mis. Aksioma Fundamental">
                                </div>
                                <div class="flex gap-4 pt-4">
                                    <button type="submit" class="flex-1 py-3 bg-primary text-on-primary font-bold rounded-xl">Tambah Sub-bab</button>
                                    <button type="button" onclick="this.closest('#add-sub-chapter-modal-{{ $chapter->id }}').classList.add('hidden')" class="flex-1 py-3 bg-surface-container-high text-on-surface font-semibold rounded-xl">Batal</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <div class="space-y-6">
        <div class="bg-surface-container-low p-6 rounded-2xl border border-outline-variant/10 shadow-sm">
            ...
            <div class="space-y-4 text-sm">
                ...
                <div>
                    <span class="text-on-surface-variant">Akses:</span>
                    <span class="font-bold text-primary float-right">Selamanya</span>
                </div>
                <div>
                    <span class="text-on-surface-variant">Status:</span>
                    <span class="font-bold {{ $course->is_approved ? 'text-primary' : 'text-tertiary' }} float-right">
                        {{ $course->is_approved ? 'Aktif' : 'Draf' }}
                    </span>
                </div>
            </div>
...
<div id="edit-course-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-on-surface/40 backdrop-blur-sm">
    <div class="bg-surface-container-lowest p-8 rounded-3xl w-full max-w-lg shadow-2xl border border-outline-variant/15 max-h-[90vh] overflow-y-auto">
        <h3 class="text-xl font-bold mb-6 text-on-surface">Perbarui Metadata Kelas</h3>
        <form action="{{ route('instructor.courses.update', $course->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            
            <div class="space-y-2">
                <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-widest">Judul Kelas</label>
            ...
            <div class="space-y-2">
                <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-widest">Harga (USD)</label>
            ...
            <div class="space-y-2">
                <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-widest">Deskripsi</label>
            ...
            <div class="space-y-2">
                <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-widest">Thumbnail Baru (Opsional)</label>
            ...
            <div class="flex gap-4 pt-4">
                <button type="submit" class="flex-1 py-3 bg-primary text-on-primary font-bold rounded-xl shadow-lg shadow-primary/20">Perbarui Kelas</button>
                <button type="button" onclick="document.getElementById('edit-course-modal').classList.add('hidden')" class="flex-1 py-3 bg-surface-container-high text-on-surface font-semibold rounded-xl">Batal</button>
            </div>
        </form>
    </div>
</div>
...
<div id="add-chapter-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-on-surface/40 backdrop-blur-sm">
    <div class="bg-surface-container-lowest p-8 rounded-2xl w-full max-w-md shadow-2xl border border-outline-variant/15">
        <h3 class="text-xl font-bold mb-6 text-on-surface">Tambah Bab Baru</h3>
        <form action="{{ route('instructor.courses.chapters.store', $course->id) }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2">Judul Bab</label>
                <input type="text" name="title" required class="w-full px-4 py-3 bg-surface-container-low border-none rounded-xl focus:ring-2 focus:ring-primary/20" placeholder="mis. Pengantar Logika">
            </div>
            <div class="flex gap-4 pt-4">
                <button type="submit" class="flex-1 py-3 bg-primary text-on-primary font-bold rounded-xl">Tambah Bab</button>
                <button type="button" onclick="document.getElementById('add-chapter-modal').classList.add('hidden')" class="flex-1 py-3 bg-surface-container-high text-on-surface font-semibold rounded-xl">Batal</button>
            </div>
        </form>
    </div>
</div>
