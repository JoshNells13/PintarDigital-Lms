...
@section('title', 'My Courses | Sanctuary Instructor')
@section('header', 'Your Educational Sanctuary')

@section('content')
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h2 class="text-xl font-bold tracking-tight">Manage Your Courses</h2>
    ...
    @if($courses->isEmpty())
        <div class="bg-surface-container-low p-16 rounded-3xl text-center border-2 border-dashed border-outline-variant/10">
            ...
            <p class="text-on-surface-variant font-medium text-lg">Your library is currently empty.</p>
            <p class="text-xs text-on-surface-variant/60 mb-8 max-w-xs mx-auto">Start sharing your wisdom by creating your first course on the sanctuary.</p>
            <a href="{{ route('instructor.courses.create') }}" class="text-primary font-bold hover:underline">Create a Course Now</a>
        </div>
    @else
                    ...
                    <div class="h-40 bg-surface-container-high relative">
                        ...
                        <div class="absolute top-4 right-4">
                            @if($course->is_approved)
                                <span class="px-2 py-1 bg-primary/10 text-primary text-[10px] font-bold uppercase rounded-md backdrop-blur-md">Live</span>
                            @else
                                <span class="px-2 py-1 bg-tertiary/10 text-tertiary text-[10px] font-bold uppercase rounded-md backdrop-blur-md">Draft</span>
                            @endif
                        </div>
                    </div>
                    <div class="p-6">
                        ...
                        <div class="flex gap-2">
                            ...
                            <form action="{{ route('instructor.courses.destroy', $course->id) }}" method="POST" class="inline" onsubmit="return confirm('Ensure all data is backed up. Deleting is permanent. Proceed?')">
                                @csrf
                                @method('DELETE')
